...
const SUPABASE_URL  = "https://fkhtiqixrkopznljoocd.supabase.co";   
const SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImZraHRpcWl4cmtvcHpubGpvb2NkIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Nzg1NjcxOTcsImV4cCI6MjA5NDE0MzE5N30.lGlZyI3kjdy-dqQIosPROPSMlBfNlHp-iS0AkDc6u-o"; 
...
export async function fetchUsers() {
    const rows = await sbFetch("owoj_users?select=*&order=rotation_order.asc");
    return rows.map(r => ({
        id: r.id,
        name: r.name,
...
export async function insertUser(user) {
    const rows = await sbFetch("owoj_users", {
        ...
        body: JSON.stringify({
            id: user.id,
            name: user.name,
            start_juz: user.startJuz,
            rotation_order: user.rotationOrder
        })
    });
    const r = rows[0];
    return {
